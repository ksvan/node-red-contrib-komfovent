...
Return values are either the mode if success, or the error returned. (add error value info)
	...
	{ error: true, result: 'wrong password', unit: node.komfoUser.ip }
	{ error: false, result: 'auto', unit: node.komfoUser.ip }
	{ error: false, result: '16.6', unit: node.komfoUser.ip } (future release with property extraction, ie outside temperature etc)
...
The node is tested against the C6 controller running 1.3.14.14 firmware. 
